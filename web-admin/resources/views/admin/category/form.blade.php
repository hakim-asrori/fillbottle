        ...
        <div class="col-lg-6">
            <div class="card shadow mb-4">
                <div class="card-header">
                    <h4 class="m-0 font-weight-bold text-primary">{{$formTitle}} Category</h4>
                </div>
                <div class="card-body">
                    @if(!empty($category))
                    ...
                    <div class="form-group">
                        ...
                        'class' => 'form-control',
                        'selected' => !empty(old('parent_id')) ? old('parent_id') : (!empty($category['parent_id']) ? $category['parent_id'] : ''), 'placeholder' => '-- Choose Category --']) !!}
                        @error('parent_id')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
